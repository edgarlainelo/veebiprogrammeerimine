...
function signUp($name, $surname, $email, $gender, $birthDate, $password){
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  //kui salasõna klapib
		  $stmt->close();
		  $stmt = $conn->prepare("SELECT id, firstname, lastname, password, email FROM vpusers2 WHERE email=?");
		  echo $conn->error;
		  $stmt->bind_param("s", $email);
		  $stmt->bind_result($idFromDb, $firstnameFromDb, $lastnameFromDb, $passwordFromDbNew, $emailFromDb);
		  $stmt->execute();
		  $stmt->fetch();
		  ...
		  $_SESSION["userPassword"] = $passwordFromDbNew;
 ...
 function newPassword($password){
   ...
   $stmt->bind_param("s", $_SESSION["userEmail"]);
	...
	if($stmt->fetch()){
         $stmt->close();
		 $stmt = $conn->prepare("UPDATE vpusers2 SET password = ? WHERE id = ?");
		 echo $conn->error;
		 ...
		 if($stmt->execute()){
			 $noticeParool = "Password on uuendatud";
		 } else {
			 $noticeParool = "Passwordi uuendamisel tekkis tõrge! " .$stmt->error;
		 }
